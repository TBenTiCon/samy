      ...
      dealWrapper.innerHTML += `
            <div class="result-item">
					<div class="div-block-12">
						<div class="countdown-container ${isDownable ? "show" : "hidden"}" id="I${
        deal._id
      }">
						...
						<div class="result-content">
							<div>
								<h6 class="result_time">${dayToDate(deal.date)} | ${convertToTime(
        deal.time
      )} Uhr</h6>
								...
								<div class="div-block-7">
									...
									<div class="result-meta" data-interacted="false">
										...
										<div class="likebtncon">
											...
											<img src="/images/like.svg" loading="lazy" alt="" class="like_img" data-_id="${
                        deal._id
                  ...
                  deal._id
						...
						<div class="todealbtn mobile">
							<h4 class="heading-8"><a href=${
                deal.afLink
              } style="text-decoration: none">Zum Deal</a></h4>
						</div>
					</div>
				</div> `;
    ...
    el.addEventListener("click", (e) => {
      if (e.target?.className === "like_img") {
        if (e.target.dataset.interacted === "false") {
          ...
          if (
            e.target.parentElement.parentElement.dataset.interacted === "false"
          ) {
            console.log("like2");
...
window.onload = () => {
  ...
  searchDeal(undefined, undefined, undefined, now, undefined, counter).then(
    (deals) => {
      renderDeals(deals);
    }
  );
...
searchForm.addEventListener("submit", (e) => {
  ...
  heading4.innerHTML = `Ergebnisse für <strong>${
    key ? searchBar.value : "alle"
  }</strong>`;

  searchDeal(
    undefined,
    undefined,
    key ? searchBar.value : undefined,
    now,
    undefined,
    counter
  ).then((deals) => {
...
fCatCon.addEventListener("click", (e) => {
  if (e.target.className === "link") {
    ...
    searchDeal(key, undefined, undefined, now, undefined, counter).then(
      (deals) => {
        renderDeals(deals);
      }
    );
